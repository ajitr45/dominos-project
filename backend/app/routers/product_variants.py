...
def create_product_variant_api(variant_data: ProductVariantCreate, db: Session = Depends(get_db),
...
def update_product_variant_api(
    ...
    try:
        return update_product_variant(db, variant, variant_data)
...
def deactivate_product_variant_api(variant_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
...
def activate_product_variant_api(variant_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
    ...
    if variant.is_active:
        raise HTTPException(
            ...
            detail="Product variant is already active",
        )

    try:
        activated_variant = activate_product_variant(
            db,
            variant,
        )
